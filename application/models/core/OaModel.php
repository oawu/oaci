...
class OaModel extends ActiveRecordModel {
  ...
  public function recycle () {
    ...
    if (!(count ($delete_columns = array_keys ($delete_model::table ()->columns)) && count ($ori_columns = array_keys ($ori_model::table ()->columns)) && (count ($ori_columns) == (count ($delete_columns) - 1)))) showError ('The origin model or delete mode columns error!');
  ...
  public static function recycle_all ($conditions = array ()) {
    ...
    for ($offset = isset ($conditions['offset']) ? $conditions['offset'] : 0; $offset < $count; $offset += $conditions['limit']) { 
      ...
      if (count ($ori_objects)) {
        $sqls = array_filter (array_map (function ($ori_object) use ($ori_columns, $origin_id) { $sql = array (); foreach ($ori_columns as $ori_column) $sql[$ori_column == 'id' ? $origin_id : $ori_column] = '"' . $ori_object->$ori_column . '"'; return count ($sql) ? $sql : null; }, $ori_objects));
    ...
    if ($count = count ($ori_ids)) $ori_model::delete_all (array ('conditions' => array ('id IN (?)', $ori_ids)));
      ...
      case 'all':
        ...
        for ($offset = isset ($conditions['offset']) ? $conditions['offset'] : 0; $offset < $count; $offset += $conditions['limit']) { 
          ...
          if (count ($delete_objects)) {
            ...
            if (count ($sqls) && count ($delete_objects) && count ($delete_ids = array_merge ($delete_ids, array_map (function ($delete_object) { return $delete_object->id; }, $delete_objects)))) $ori_model::query ("INSERT INTO `" . $ori_model::table ()->table . "`(" . implode (",", array_map (function ($ori_column) { return "`" . $ori_column . "`"; }, $ori_columns)) . ") VALUES" . implode (',', array_map (function ($sql) { return "(" . implode (",", $sql) . ")"; }, $sqls)));
          }
        }
        if ($count = count ($delete_ids)) $delete_model::delete_all (array ('conditions' => array ('id IN (?)', $delete_ids)));
        return $count ? true : false;
        break;
